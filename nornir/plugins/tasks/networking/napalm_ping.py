...
def napalm_ping(
    ...
    Arguments:
      ...
      (optional) timeout – Maximum seconds to wait after sending final packet. (default=2) (type=integer)
    ...
    result = device.ping(destination=dest, source=source, ttl=ttl, timeout=timeout, size=size, count=count, vrf=vrf)
